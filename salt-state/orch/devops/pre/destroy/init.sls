...
# ACTION ON MASTER
pre destroy {{group}} orchestration state hook message:
  salt.function:
    - name: cmd.run
    - tgt: 'saltmaster'
    - arg:
      - echo test pre destroy group orchestrate state, nodes {{minion_target}}

# ACTION ON MINION
pre destroy {{group}} orchestration state hook:
