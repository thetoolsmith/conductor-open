# pre provision group wide orchestration state. gets before new instance is brought online
# REQUIRED PARAMETERS:
...
pre provision {{group}} orchestration state hook message:
  salt.function:
    - name: cmd.run
    - tgt: 'saltmaster'
    - arg:
      - echo test pre provision group orchestrate state, nodes {{minion_target}}

pre provisioning {{group}} orchestration state hook:
