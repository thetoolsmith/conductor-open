# post startup state group wide orchestration state. gets run when a new instance is brought online
# REQUIRED PARAMETERS:
...
post startup {{group}} orchestration state hook message:
  salt.function:
    - name: cmd.run
    - tgt: 'saltmaster'
    - arg:
      - echo test post startup group orchestrate state, nodes {{minion_target}}

post startup {{group}} orchestration state hook:
