# EXAMPLE
# SPECIFIC TO COMBINED ENV, GROUP AND ROLE 
# 
# REQUIRED PARAMETERS:
...
{{role_target}} pre destroy orchestration test all:
  {% set target = "'L@" + minion_target + " and ( G@role:" + role_target + " or G@composite.role:*" + role_target + "* )'" %}
  salt.state:
    - tgt: {{ target }}
    - tgt_type: compound
    - sls: 
      - salty.nifi.pre_destroy_orch_test
    - pillar: {
        target-minion: {{target}}
      } 

# PERFORM SOME ACTION ON SALT MASTER
{{role_target}} salty pre destroy orchestration state hook message:
  salt.function:
    - name: cmd.run
    - tgt: 'saltmaster'
    - arg:
