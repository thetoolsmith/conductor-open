...
{% if service_installed == True %}
  ...
  {% if (grains['role']|lower == role) or
    ...
    {% set consulconfig_path = salt['pillar.get']('salty.role:consul-server:config-location', None) %}
...
re-generate local {{role}} config:
  module.run:
    - name: common_consul.create_salty_server_config
